...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id = $_POST['id'];
    $varenavn = trim($_POST['varenavn']);

    if ($varenavn !== '') {
        $stmt = $pdo->prepare("UPDATE lagerbeholdning SET varenavn = :v WHERE id = :id");
        $stmt->execute([':v' => $varenavn, ':id' => $id]);
        $message = "Varenavn opdateret.";
    } else {
        $message = "Varenavn må ikke være tomt.";
    }
}
            ...
            <?php foreach ($rows as $r): ?>
                ...
                <dialog id="edit-<?= $r['id'] ?>" class="p-3 rounded">
                    <form method="POST">
                        ...
                        <label>Nyt varenavn</label>
                        <input type="text" name="varenavn" class="form-control mb-3"
                               value="<?= htmlspecialchars($r['varenavn']) ?>" required>
